  ...
  function formatCurrency(rawValue, rawCurrencyChoice) {
    ...
    if (rawValue) {
      ...
      if (str) {
        ...
        // Se o usuário digitou ambos (ex: 1.000,50 ou 1,000.50)
        if (lastDot > -1 && lastComma > -1) {
          if (lastDot > lastComma) {
            str = str.replace(/,/g, ''); // Ponto é decimal
          } else {
            str = str.replace(/\./g, '').replace(',', '.'); // Vírgula é decimal
          }
        } 
        // Só tem vírgula (ex: 100,50)
        else if (lastComma > -1) {
          str = str.replace(',', '.');
        } 
        // Só tem ponto (ex: 100.50 ou 1.000)
        else if (lastDot > -1) {
          let parts = str.split('.');
          // Se tiver exatamente 3 casas após o ponto (ex: 1.000), assumimos que é milhar
    ...
    // Retorna formatado no padrão local com o símbolo correto
